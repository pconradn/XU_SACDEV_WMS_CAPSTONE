            ...
            <div class="bg-white shadow rounded p-6">
                <form method="POST" action="{{ route('admin.organizations.update', $organization) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input name="name" value="{{ old('name', $organization->name) }}" class="mt-1 w-full border rounded p-2" required>
                        @error('name') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium">Acronym (optional)</label>
                        <input name="acronym" value="{{ old('acronym', $organization->acronym) }}" class="mt-1 w-full border rounded p-2">
