<x-app-layout>
    <div class="mx-auto max-w-4xl px-4 py-6">
        <div class="mb-4">
            ...
            <p class="mt-1 text-sm text-slate-600">Select the target School Year to manage the members list.</p>
        </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <form method="POST" action="{{ route('org.b4.members-list.setTargetSy') }}">
                @csrf
        ...
        @if($list)
            <div class="mt-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <div class="text-sm text-slate-600">Last updated</div>
                        <div class="font-semibold text-slate-900">{{ $list->updated_at?->format('M d, Y h:i A') }}</div>
                    </div>

                    <a href="{{ route('org.b4.members-list.edit') }}"
