                ...
                @php
                    ...
                    $approvalTasks = $pendingTasks
                        ...
                        ->values();
                    $approvalTasksByProject = $approvalTasks
                ...
                <div class="lg:col-span-2 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <div class="p-5 space-y-4 max-h-[500px] overflow-y-auto pr-2">
                        ...
                        @if($approvalTasksByProject->count())
                            @foreach($approvalTasksByProject as $projectId => $tasks)
                                @php
                                    ...
                                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                        <div class="flex items-start gap-3 min-w-0">
                                            ...
                                            <div class="min-w-0">
                                                <div class="flex flex-wrap items-center gap-2">
                                                    <div class="text-sm font-semibold text-slate-900">
                                                        {{ $project->title ?? 'Project' }} needs review
                                                    </div>
                                                    ...
                                                    <span class="rounded-full border border-indigo-200 bg-white px-2 py-0.5 text-[10px] font-semibold text-indigo-700">
                                                        {{ $tasks->count() }} {{ $tasks->count() > 1 ? 'documents' : 'document' }}
                                                    </span>
                                                </div>

                                                <p class="mt-1 text-xs leading-5 text-slate-600">
                                                    The following document{{ $tasks->count() > 1 ? 's are' : ' is' }} waiting for your approval, review, or signature.
                                                </p>
