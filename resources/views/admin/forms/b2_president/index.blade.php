<x-app-layout>
    <div class="mx-auto max-w-6xl px-4 py-6">
        <div class="mb-4">
            <h2 class="text-xl font-semibold text-slate-900">B-2 President Registrations (SACDEV)</h2>
                            ...
                            <tr>
                                <td class="py-3 px-4">
                                    ...
                                    <div class="text-xs text-slate-500">
                                        Encoded by: {{ $r->encoded_by_user_id ?? '—' }}
                                    </div>
                                </td>

                                <td class="py-3 px-4">
                                    {{ $r->targetSchoolYear->label ?? $r->target_school_year_id }}
                                </td>
                                ...
                                <td class="py-3 px-4 text-slate-600">
                                    {{ $r->submitted_at ? $r->submitted_at->format('M d, Y') : '—' }}
                                </td>

                                <td class="py-3 px-4 text-right">
                                    <a href="{{ route('admin.b2.president.show', $r->id) }}"
                                       class="inline-flex justify-center rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50">
                                        Open
                                    </a>
                                </td>
